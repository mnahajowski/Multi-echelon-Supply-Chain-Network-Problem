...
const double CMscnProblem::MAXIMUM_COST_UD = 5;
const double CMscnProblem::MINIMUM_COST_UF = 1;
const double CMscnProblem::MAXIMUM_COST_UF = 5;
const double CMscnProblem::MINIMUM_COST_UM = 1;
const double CMscnProblem::MAXIMUM_COST_UM = 5;

const double CMscnProblem::MINIMUM_COST_SD = 1;
const double CMscnProblem::MAXIMUM_COST_SD = 1000;
const double CMscnProblem::MINIMUM_COST_SF = 1;
const double CMscnProblem::MAXIMUM_COST_SF = 1000;
const double CMscnProblem::MINIMUM_COST_SM = 1;
const double CMscnProblem::MAXIMUM_COST_SM = 1000;
const double CMscnProblem::MINIMUM_COST_SS = 1;
const double CMscnProblem::MAXIMUM_COST_SS = 1000;
...
void CMscnProblem::getMaxMin(int index, double *pdSolution, double *error, int length) {
	double minim = 0;
	double maxim = 0;
	if (index < 0 || index > this->providers*this->factories + this->factories*this->magazines + this->magazines*this->shops)
		*error = INVALID_VALUE;
		...
		if (minmaxxd[index%this->providers][(index%this->factories)+1] > maxim)
			maxim = minmaxxd[index%this->providers][(index%this->factories)+1];
	}
	else if (index < this->providers*this->factories + this->factories*this->magazines) {
		index = index - this->providers*this->factories;
		if (minmaxxd[index%this->factories][(index%this->magazines)+1] > maxim)
			maxim = minmaxxd[index%this->factories][(index%this->magazines)+1];
	}
		...
		if (minmaxxd[index%this->magazines][(index%this->shops)+1] > maxim)
			maxim = minmaxxd[index%this->magazines][(index%this->shops)+1];
	}
...
bool CMscnProblem::bConstraintSatisfied(double *pdSolution, double *error, int length) {
