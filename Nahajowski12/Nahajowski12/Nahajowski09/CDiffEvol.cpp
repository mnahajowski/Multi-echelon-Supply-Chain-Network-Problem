...
const int CDiffEvol::MAXIMUM_TRIES = 10;
...
double *CDiffEvol::evaluate(CMscnProblem *c1) {
	...
	double *dbls_best = new double[size];
	//std::cout << " " << size << "\n";
	double test;
...
double *CDiffEvol::optimize(double seconds) {
	this->timer.cStart();
	std::cout << "DE\n";
	double maxim = -100000;
	double first_maxim = -100000;
	double test2 = 0;
	...
	for (int i = 0; i < this->length; i++) {
		this->solutions[i] = new double[size];
		for (int j = 0; j < size; j++) {
			solutions[i][j] = randomizer1.getDouble(MINIMUM, this->problem->getMaxMin(j, solutions[i], &test, size));
			//std::cout << solutions[i][j] << " ";
			//std::cout << " Max " << c1->getMaxMin(j, solutions[i], &test, size) << " ";
		}
		//std::cout << "\n";
	...
	for (int i = 0; i < this->length; i++) {
		...
		//std::cout << "PROFIT DLA " << i << " = " << c1->dGetQuality(solutions[i], size) << "\n";
		if (this->problem->dGetQuality(solutions[i], size) > maxim) {
			maxim = this->problem->dGetQuality(solutions[i], size);
			first_maxim = this->problem->dGetQuality(solutions[i], size);
			for (int j = 0; j < size; j++) {
				dbls_best[j] = solutions[i][j];
			}
		}
	...
	while (this->timer.cResult() < seconds) {
		...
		for (int k = 0; k < this->length; k++) {
			for (int i = 0; i < size; i++) {
				dbls1[i] = randomizer2.getDouble(MINIMUM, this->problem->getMaxMin(i, dbls1, &test, size));
				dbls2[i] = randomizer3.getDouble(MINIMUM, this->problem->getMaxMin(i, dbls2, &test, size));
				dbls3[i] = randomizer4.getDouble(MINIMUM, this->problem->getMaxMin(i, dbls3, &test, size));
			}
			...
			if (individualsAreDifferent(&solutions[k], &dbls1, &dbls2, &dbls3, size)) {
				for (int i = 0; i < size; i++) {
					...
					if (dis(gen) < CROSS_PROB) {
						...
						else if (indNew[i] > this->problem->getMaxMin(i, indNew, &test, size))
							indNew[i] = this->problem->getMaxMin(i, indNew, &test, size);
					}
					else {
						indNew[i] = solutions[k][i];
					}
				}
				//std::cout << "status " << this->problem->bConstraintSatisfied(solutions[k], &test, size) << "\n";
				double y = this->problem->dGetQuality(solutions[k], size);
				//std::cout << "tu5";
				double x = this->problem->dGetQuality(indNew, size);
				//std::cout << "tu4";
				...
				if (x > y) {
					...
					if (this->problem->dGetQuality(indNew, size) > maxim) {
						maxim = this->problem->dGetQuality(indNew, size);
						for (int j = 0; j < size; j++) {
							dbls_best[j] = indNew[j];
						}
					}
	...
	for (int i = 0; i < this->length; i++) {
		...
		std::cout << "PROFIT DLA " << i << " = " << this->problem->dGetQuality(solutions[i], size);
		std::cout << "\n";
	}
	std::cout << "MAXIM = " << maxim << "\n";
	...
	for (int i = 0; i < this->length; i++) {
		...
		std::cout << "PROFIT DLA " << i << " = " << this->problem->dGetQuality(first_solutions[i], size);
		std::cout << "\n";
	}
	std::cout << "FIRST_MAXIM = " << first_maxim << "\n";
	for (int i = 0; i < size; i++) {
	...
	delete dbls3;
	delete indNew;
	std::cout << "czas " << this->timer.cResult() << "\n";
	//return &solutions;
	return dbls_best;
